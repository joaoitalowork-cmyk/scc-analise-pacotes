...
//
// @updateURL    https://raw.githubusercontent.com/SEU_USUARIO/SEU_REPO/main/scc_analise_pacotes.user.js
// @downloadURL  https://raw.githubusercontent.com/SEU_USUARIO/SEU_REPO/main/scc_analise_pacotes.user.js
// ==/UserScript==
...
(function () {
  ...
  async function fetchTBR(tbr) {
    ...
    const rtoId   = linked[0] || null;
    let rtoEvents = [];
    if (rtoId) {
      const r = await fetchHistory(rtoId).catch(() => null);
      rtoEvents = r?.packageHistory || r?.responseBody?.packageHistory || [];
    }
    return { tbr, events, rtoId, rtoEvents };
  }
  ...
  function buildTable(events, tid, isRTO, hlLogins) {
    if (!events.length)
      return `<p style="color:#888;font-style:italic;margin:8px 0">Nenhum evento encontrado para ${tid}.</p>`;
    const rtoLbl = isRTO ? `<strong style="color:#6c3483">&#8617; RTO:</strong> ` : '';
    ...
    const rows   = events.map((ev, i) => {
      ...
      const assocCell = isEdsp
        ...
        : isAmazon
          ...
          : td(aso);
      return `<tr style="background:${bg};border-bottom:1px solid #e0e0e0">
  ...
  function renderMissingCard(result, container) {
    const { tbr, rtoId, rtoEvents } = result;
    ...
    const rtoTag  = rtoId
      ? `<span style="background:#9b59b6;color:#fff;padding:2px 8px;border-radius:12px;font-size:10px;margin-left:6px">RTO: ${rtoId}</span>` : '';
    ...
    if (isVal) {
      ...
      bodyContent = `
        ...
        ${postMissingSection}
        ${rtoId && rtoEvents.length ? `<div style="margin-top:14px;border-top:2px dashed #9b59b6;padding-top:12px">${buildTable(rtoEvents, rtoId, true, true)}</div>` : ''}`;
    ...
    card.innerHTML = `
      <div class="scc-chdr" style="background:${hdrBg};color:${hdrFg}"
        ...
        <span style="display:flex;align-items:center;gap:8px">
          ${typeBdg} <strong>${tbr}</strong> ${last ? badge(last.packageState) : ''} ${rtoTag}
  ...
  function renderGeralCard(result, container) {
    const { tbr, events, rtoId, rtoEvents } = result;
    const last   = events[events.length - 1];
    const rtoTag = rtoId
      ? `<span style="background:#9b59b6;color:#fff;padding:2px 8px;border-radius:12px;font-size:10px;margin-left:8px">RTO: ${rtoId}</span>` : '';
    const card = document.createElement('div');
    card.className = 'scc-card';
    card.innerHTML = `
      <div class="scc-chdr"
           onclick="(function(el){var b=el.nextElementSibling;b.style.display=b.style.display==='none'?'':'none';})(this)">
        <span>${tbr} ${last ? badge(last.packageState) : ''}${rtoTag}</span>
        <span style="font-size:11px;color:#555">${events.length} evento(s) &#9662;</span>
      </div>
      <div class="scc-cbody">
        ${buildTable(displayOrder(events), tbr, false, false)}
        ${rtoId && rtoEvents.length ? `<div style="margin-top:14px;border-top:2px dashed #9b59b6;padding-top:12px">${buildTable(displayOrder(rtoEvents), rtoId, true, false)}</div>` : ''}
      </div>`;
    container.appendChild(card);
  }
  ...
  function downloadCSV(data, tipo) {
    ...
    data.forEach(r => {
      ...
      if (tipo !== 'MISSING' && r.rtoId && r.rtoEvents.length) {
        buildXLRows(displayOrder(r.rtoEvents), r.rtoId+' (RTO)', tipo, null)
    ...
    ov.innerHTML = `
      <div id="scc-type-modal" role="dialog" aria-modal="true">
        ...
        <div id="scc-type-body">
          ...
          <div class="scc-tc scc-tc-soon" data-type="LOST">
            <div class="scc-tc-icon">&#128683;</div>
            <div class="scc-tc-name">Lost <span class="scc-soon-tag">Em breve</span></div>
            <div class="scc-tc-desc">Análise de pacotes perdidos</div>
          </div>
      ...
      await Promise.all(tbrs.slice(i, i + 10).map(tbr =>
        fetchTBR(tbr)
          .then(r => {
            ...
            currentType === 'MISSING' ? renderMissingCard(r, res) : renderGeralCard(r, res);
          })
